...
(function(window, Reveal, hljs) {
  ...
  function clearPreviousFocus() {
    forEach(currentSlide.querySelectorAll("pre code .line.focus"), function(
      line
    ) {
  ...
  function focusFragment(fragment) {
    ...
    if (scrollToFocused && scrollLineTop != null) {
      ...
      scrollLineBottom =
        code[scrollLineBottom].offsetTop + code[scrollLineBottom].clientHeight;
      codeParent.scrollTop =
        scrollLineTop -
        (codeParent.clientHeight - (scrollLineBottom - scrollLineTop)) / 2;
    }
  }
